...
// Usage
const adminOnly = createRoleBasedAccess(['admin'])
```
...
### Admin-Only Collection (Template)
...
### Customer-Owned Collection (Template)

```typescript
export const CustomerOwnedCollection: CollectionConfig = {
  slug: 'orders',
  access: {
    read: ({ req: { user } }) => {
      if (!user) return false
      if (user.role === 'admin') return true
      return { orderedBy: { equals: user.id } }
    },
    update: ({ req: { user } }) => user?.role === 'admin',
    delete: ({ req: { user } }) => user?.role === 'admin',
  ...
  fields: [
    { name: 'orderedBy', type: 'relationship', relationTo: 'users', required: true },
      ...
      access: {
        read: ({ req: { user }, context }) => {
          ...
          if (context.canReadSecret === undefined) {
            context.canReadSecret = user?.role === 'admin'
          }
